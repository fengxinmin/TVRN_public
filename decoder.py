...
import numpy as np
from utils.hevc_feature_decoder import HevcFeatureReader, HevcFeatureReaderYUV444
import torchvision.transforms as T

def decode_compressed_video(filename, format='yuv420p'):

    if format == 'yuv420p':
        reader = HevcFeatureReader(filename, nb_frames=None, n_parallel=4)
    elif format == 'yuv444p':
        # reader = HevcFeatureReaderYUV444(filename, nb_frames=None, n_parallel=4)
...
if __name__ == "__main__":
    # yuv420
    # /root/miniconda/envs/uprnet/bin/ffmpeg -y -f rawvideo -s 1920x1080 -pix_fmt yuv420p -i /data/fengxm/vimeo90k/HEVC_B_reorg/HEVC_B_src/MarketPlace_1920x1080_60fps_8bit_420.yuv -c:v libx265 -preset veryfast -tune zerolatency -frames:v 10 -x265-params cqp=32 /output/demo.h265
    
    # yuv444
    # /model/fengxm/VRN/mp4_demo/outputvideocpucvb.h265
    
    video = "/output/outputvideocpucvb.h265" # yuv444p
    format = 'yuv420p'
    
    # video = "/model/fengxm/VRN/mp4_demo/outputvideocpucvb.h265"  # yuv444
    # format = "yuv444p"
    cvf = decode_compressed_video(video, format=format)  # cvf: compressed video features
    # # features
    # covos_features = {
    #     "nb_frames": num_frames,
    #     "frame_type": frame_type,
    ...
    plt.imsave('/code/demo.png', cvf['residual'][0,:,:])
    pass