...
def parse(opt_path, is_train=True):
    ...
    # checkpoints path
    opt['path']['models'] = os.path.join(opt['path']['models'], opt['name'])
    ...
    for key, path in opt['path'].items():
        if path and key in opt['path'] and key not in ['strict_load', 'strict_load_G']:
    ...
    if is_train:
        ...
        opt['path']['training_state'] = osp.join(experiments_root, 'states')
        # opt['path']['log'] = '/output/log'
