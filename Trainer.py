...
from models_EMA.loss import *
from models_EMA.warplayer import warp
...
class Model:
    ...
    def load_model(self, name=None, rank=0, path=None):
        def convert(param):
            return {
            k.replace("module.", ""): v
        ...
        if rank <= 0 :
            ...
            print("path: ", path)
            self.net.load_state_dict(convert(torch.load(path)))
    ...
    def inference(self, img0, img1, TTA = False, timestep = 0.5, fast_TTA = False):
        ...
        if fast_TTA:
            ...
            # macs, params = profile(self.net, inputs=(input, timestep))
