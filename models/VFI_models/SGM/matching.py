...
def forward_warp(tenIn, tenFlow, z=None):
    if z is None:
        ...
        z = torch.where(z == 0, -20 * torch.ones_like(z), torch.ones_like(z))
...
def build_hole_mask(img_template, flow_tA, flow_tB):
    ...
    with torch.no_grad():
        ...
        hole_mask = torch.where(out == 0, torch.zeros_like(out), torch.ones_like(out))
    return hole_mask
...
def flow_shift(flow_fix, timestep, num_key_points=None, select_topk=False):
    ...
    z = torch.where(flow_fix == 0, torch.zeros_like(flow_fix), torch.ones_like(flow_fix)).detach().sum(1, keepdim=True) / 2
    zt0, zt1 = z[B:], z[:B]
    ...
    if select_topk and num_key_points != -1:
        ...
        warp_map_topk = torch.where(warp_map_topk != 0, torch.ones_like(warp_map_topk), torch.zeros_like(warp_map_topk))
