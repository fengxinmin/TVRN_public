...
from models_cvrs.module.general_module import make_layer,ResidualBlock3D_NoBN
import torch
import torch.nn as nn
from models_cvrs.module.general_module import make_layer
...
from models_cvrs.module.my_3D_module import BasicLayer
...
class Rescaler_MixNet(nn.Module):
    ...
    def forward(self, imgs,size, rev=False):
        if rev == False:
            return self.downet(imgs,target_size=size)
        else:
            B,C,T,H,W = imgs.shape
            up_imgs = self.upnet(imgs,target_size =size)
            return up_imgs
