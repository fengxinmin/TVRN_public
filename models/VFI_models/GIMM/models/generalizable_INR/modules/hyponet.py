...
class HypoNet(nn.Module):
    ...
    def __init__(self, config: HypoNetConfig, add_coord_dim=32):
        ...
        if len(self.hidden_dims) == 1:
            if OmegaConf.is_config(self.hidden_dims):
                hidden_list = OmegaConf.to_object(self.hidden_dims)
            else:
                hidden_list = self.hidden_dims
            self.hidden_dims = hidden_list * (self.num_layer - 1)
        else:
