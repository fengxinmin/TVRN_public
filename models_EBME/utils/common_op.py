...
def conv(in_planes, out_planes, kernel_size=3, stride=1, padding=1, dilation=1):
    return nn.Sequential(
        nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, stride=stride,
            padding=padding, dilation=dilation, bias=True, padding_mode='replicate'),
        nn.PReLU(out_planes)
    )
...
def conv2(in_planes, out_planes, stride=2):
    return nn.Sequential(
        nn.Conv2d(in_planes, out_planes, 3, stride, 1, padding_mode='replicate'),
        nn.PReLU(out_planes),
        nn.Conv2d(out_planes, out_planes, 3, 1, 1, padding_mode='replicate'),
        nn.PReLU(out_planes)
    )
