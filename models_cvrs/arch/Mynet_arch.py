...
from models_cvrs.module.general_module import DepthwiseSeparableConv3d
from models_cvrs.module.general_module import make_layer,ResidualBlock3D_NoBN,DepthwiseTransSeparableConv3d
import torch
import torch.nn as nn
import torch.nn.functional as F
from models_cvrs.module.general_module import ResBlock,make_layer
from models_cvrs.module.sample_module import ModuleNet3D
from models_cvrs.module.Quantization import Quantize_ste
from models_cvrs.module.inv_module import D2DTInput
...
class RescalerNet(nn.Module):
    ...
    def forward(self, imgs,size, rev=False):
        if rev == False:
            out = self.downet(imgs,target_size = size)
            out = torch.clamp(out, 0, 1)
            return out
        else:
            up_imgs = self.upnet(imgs,target_size =size).contiguous()
