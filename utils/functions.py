...
def ycbcr420_planner_to_444(yuv420_image, h, w, order=1):
    '''
    yuv420_image is (h+h/2)xwx1 YCbCr420 float numpy array, in the range of [0,1]
    return yuv444_image is hxwx3 float numpy array
    '''
    y, uv = yuv420_image[:h].reshape(1, h, w), yuv420_image[h:].reshape(2, h//2, w//2)
    yuv444_image = ycbcr420_to_444(y, uv, order=1)
    yuv444_image = rearrange(yuv444_image, 'c h w -> h w c')
    return yuv444_image
