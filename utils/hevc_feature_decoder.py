...
_HEVC_FEAT_DECODER = "/code/codes/utils/hevc.bin"
# _HEVC_FEAT_DECODER = "/output/my_hevc.bin"

_FFMPEG_SUPPORTED_DECODERS = [b".mp4", b".h265", b".265"]
...
class HevcFeatureReader:
    ...
    def __init__(self, filename, nb_frames, n_parallel):
        ...
        # TODO: 返回内容不包括Presentation Time Stamp
        ...
        if nb_frames is not None:
            ...
            # 当前版本的number of frames在nb_read_packets
            self.nb_frames = int(viddict['@nb_read_packets'])
    ...
    def _readFrame(self):
        ...
        y, uv = all_yuv_data[:self.height].astype(np.float) / 255., all_yuv_data[self.height:].reshape(2, self.height//2, self.width//2).astype(np.float) / 255.
        rgb_img = ycbcr420_to_rgb(y, uv).transpose(1,2,0)
        rgb_img = (rgb_img * 255).astype(np.uint8)
        
        y, uv = all_yuv_data_residual[:self.height].astype(np.float) / 255., all_yuv_data_residual[self.height:].reshape(2, self.height//2, self.width//2).astype(np.float) / 255.
        residual = ycbcr420_to_rgb(y, uv).transpose(1,2,0)
...
class HevcFeatureReaderYUV444:
    """Reads frame features using HevcFeatureReaderYUV444 for yuv444p format
    ...
    def __init__(self, filename, nb_frames, n_parallel):
        ...
        if self.pix_fmt not in ["yuv444p"]:
            print(self.pix_fmt)
            print(filename)
            raise NameError("Expect a yuv444p input.")
    ...
    def _read_frame_data(self):
        self.pvY_size = self.width * self.height
        self.pvU_size = self.width * self.height
        self.pvV_size = self.width * self.height
