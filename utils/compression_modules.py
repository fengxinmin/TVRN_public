...
class EntropyModel(nn.Module):
    def __init__(self, K=1, N=64, hyperprior=False, likelihood_bound=1e-9):
        super(EntropyModel, self).__init__()
        self.K = K
        self.N = N
        self.bottlenecks = nn.ModuleList([EntropyBottleneck(N, likelihood_bound=likelihood_bound) for i in range(K)])
