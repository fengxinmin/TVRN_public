...
class FFmpegReader():
    ...
    def _read_frame_data(self):
        ...
        if self.pix_fmt == 'yuv420p':
            framesize =  self.outputwidth * ((self.outputheight >> 1) + self.outputheight)
        else:
    ...
    def _readFrame(self):
        ...
        if self.pix_fmt == 'yuv420p':
            result = result.reshape(((self.outputheight >> 1) + self.outputheight), self.outputwidth)
            # result = cv2.cvtColor(cv2.cvtColor(result, cv2.COLOR_YUV420P2BGR), cv2.COLOR_BGR2YUV)
...
class FFmpegWriter():
    ...
    def writeFrame(self, im, is_yuv420p=False):
        ...
        if is_yuv420p:
            ...
            try:
                if is_yuv420p:
                    assert vid.shape[0] == 1 # 只能处理单帧
                    vid_yuv420p = cv2.cvtColor(cv2.cvtColor(vid[0], cv2.COLOR_YUV2BGR), cv2.COLOR_BGR2YUV_I420)
                    self._proc.stdin.write(vid_yuv420p.tostring())
                else:
                    self._proc.stdin.write(vid.tostring())
            except IOError as e:
                # Show the command and stderr from pipe
