...
class Vimeo90KDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		for v in self.LQ_frames_list:
			...
			# elif self.data_type == 'lmdb':
				# # modified: add multiple GT, 原始代码，有bug
			...
			img_GT = util.read_img(self.GT_env, name_a + '_{}'.format(v), (3, 256, 448))
				# img_LQ = util.read_img(self.LQ_env, name_a + '_{}'.format(v), LQ_size_tuple)
				# img_GT = util.read_img(self.GT_env, key , (3, 256, 448))
				# img_LQ = util.read_img(self.LQ_env, key , LQ_size_tuple)
			# else:
		...
		if self.random_crop:
			_, _, H, W = img_GTs.shape
			crop_size = self.crop_size
			step = 64  
			x_max = (W - crop_size) // step * step  
			y_max = (H - crop_size) // step * step  
			x_start = random.randint(0, x_max // step) * step
			y_start = random.randint(0, y_max // step) * step
			img_GTs = img_GTs[:, :, y_start:y_start+crop_size, x_start:x_start+crop_size]

		return {'GT': img_GTs, 'GT_path': key, 'index': index}
