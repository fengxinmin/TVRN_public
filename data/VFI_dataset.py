...
# video frame interpolation常用的数据集
...
class VimeoDataset(Dataset):
    ...
    def __getitem__(self, index):
        img0, gt, img1 = self.getimg(index)
        img0 = torch.from_numpy(img0.copy()).permute(2, 0, 1)[[2,1,0], :, :]
        img1 = torch.from_numpy(img1.copy()).permute(2, 0, 1)[[2,1,0], :, :]
        gt = torch.from_numpy(gt.copy()).permute(2, 0, 1)[[2,1,0], :, :]
        return {'GT': torch.stack([img0, gt, img1], dim=0)}


class SnuFilmSeptupletDataset(Dataset):
    """SNUFilmDataset"""
    def __init__(self, data_root):
        self.data_root = data_root
        self.image_root = self.data_root
        test_fn = os.path.join(self.data_root, 'test_septuplet_demo.txt')
        with open(test_fn, 'r') as f:
            self.testlist = f.read().splitlines()
            self.testlist = [item.replace('data/SNU-FILM/test', self.image_root) for item in self.testlist if item != '']                                            
        self.load_data()
    ...
    def getimg(self, index):
        ...
        #         os.path.join(self.image_root, imgpath, 'im2.png'),
        #         os.path.join(self.image_root, imgpath, 'im3.png'),
        #         os.path.join(self.image_root, imgpath, 'im4.png'),
        #         os.path.join(self.image_root, imgpath, 'im5.png'),
        #         os.path.join(self.image_root, imgpath, 'im6.png'),
        #         os.path.join(self.image_root, imgpath, 'im7.png')]
        return [cv2.imread(ele) for ele in imgpaths]
        # # Load images
        # img0 = cv2.imread(imgpaths[0])
        # gt = cv2.imread(imgpaths[1])
        # img1 = cv2.imread(imgpaths[2])
    ...
    def __getitem__(self, index):
        return {'GT': torch.stack([torch.from_numpy(ele.copy()).permute(2, 0, 1)[[2,1,0], :, :] for ele in self.getimg(index)], dim=0)}
        # img0, gt, img1 = self.getimg(index)
        # img0 = torch.from_numpy(img0.copy()).permute(2, 0, 1)[[2,1,0], :, :]
        # img1 = torch.from_numpy(img1.copy()).permute(2, 0, 1)[[2,1,0], :, :]
        # gt = torch.from_numpy(gt.copy()).permute(2, 0, 1)[[2,1,0], :, :]
