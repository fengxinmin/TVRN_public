...
###################### read images ######################
def _read_img_lmdb(env, key, size, write=False):
...
def rgb2ycbcr_tensor(img, only_y=True):
    ...
    Input:
        ...
        float, [0, 1]
    '''
    # check 使用cv2将BGR转变为YUV
    yuv_img_list = []
    img_np = rearrange(img, 'b c h w -> b h w c').cpu().numpy()
    for i in range(img.shape[0]):
        bgr_img = img_np[i]
        yuv_img = cv2.cvtColor(bgr_img, cv2.COLOR_RGB2YUV)
        yuv_img_list.append(torch.tensor(yuv_img).to(img.device))
    yuv_img = torch.stack(yuv_img_list, dim=0)
    yuv_img = rearrange(yuv_img, 'b h w c -> b c h w')
    if only_y:
        return yuv_img[:,0:1]
    else:
        return yuv_img
    ...
    #     rlt = torch.sum((torch.tensor([24.966, 128.553, 65.481]).reshape([1,3,1,1]).to(img.device) * img), dim=1) / 255.0 + 16.0
    # else:
    #     # rlt = np.matmul(img, [[24.966, 112.0, -18.214], [128.553, -74.203, -93.786],
    #                         #   [65.481, -37.797, 112.0]]) / 255.0 + [16, 128, 128]
    #     raise Exception('not implemntation')
    # # if in_img_type == np.uint8:
    # #     rlt = rlt.round()
    # # else:
    #     rlt /= 255.
    # return rlt.astype(in_img_type)
    # # return rlt.to(torch.float16)
