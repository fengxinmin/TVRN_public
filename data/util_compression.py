...
def _get_paths_from_vids(dataroot,data_list,max_size):
    ...
    if data_list is None:
        img_num = len(os.listdir(dataroot))
        vid_img_list = [ os.path.join(dataroot, f"{i:03d}.png") for i in range(1,img_num+1)]
        images += [vid_img_list]
        return images
    elif data_list[-4:] == '.txt':
        ...
        for vid_path in data_list_f.readlines():
            ...
            images += [vid_img_list]
        return images
    elif data_list[-5:] == '.json':
        seqs = []
        data_list_f = open(data_list)
        data = json.load(data_list_f)
        seqs.extend(data)
        for info in seqs:
            vid_path = info['path']
            dir_path = os.path.join(dataroot,vid_path.strip())
            vid_img_list = [ os.path.join(dataroot,vid_path.strip(), i) for i in info['frames']]
            # img_num = len(os.listdir(dir_path))
            # exit()
            # vid_img_list = [ os.path.join(dataroot,vid_path.strip(),"im"+str(i))+".png" for i in range(1,img_num+1)]
            images += [vid_img_list]
...
def get_vid_paths(data_type, dataroot, data_list=None, is_train=False):
    '''get image path list
    support lmdb or image files'''
    paths, sizes = None, None
    max_size = 200
    ...
    if dataroot is not None:
        if data_type == 'img':
            paths = sorted(_get_paths_from_vids(dataroot, data_list, max_size = max_size))
            # print(paths)
            # print("++++++++++++++")
            # print(paths[0],sizes)
            # exit()
