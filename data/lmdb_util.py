...
def vimeo90k(mode):
    ...
    if mode == 'GT':
        ...
        img_folder = '/data/liyao/vimeo/video_train/vimeo_train'
        lmdb_save_path = '/data/fengxm/vimeo90k/vimeo90k_train_GT.lmdb'
        txt_file = '/data/fengxm/vimeo90k/sep_trainlist.txt'
        H_dst, W_dst = 256, 448
    elif mode == 'LR':
        ...
        img_folder = '/data/liyao/vimeo/vimeo_train_LR'
        lmdb_save_path = '/data/fengxm/vimeo90k/vimeo90k_train_LR.lmdb'
        txt_file = '/data/fengxm/vimeo90k/sep_trainlist.txt'
        H_dst, W_dst = 64, 112
    elif mode == 'flow':
    ...
    for line in range(10000):
        folder = str(line)
        all_img_list.extend(glob.glob(osp.join(img_folder, folder, '*')))
        
        if mode == 'flow':
            for j in range(1, 4):
                keys.append('{}_{}_4_n{}'.format(folder, j))
                keys.append('{}_{}_4_p{}'.format(folder, j))
        else:
            for j in range(7):
                keys.append('{}_{}'.format(folder, j + 1))
